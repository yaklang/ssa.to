...
export const ResizeLine: React.FC<ResizeLineProps> = (props) => {
      ...
      if (distance[0] <= min || distance[1] <= max) {
        ...
        if (isVer)
          line.style.transform = `translateY(${
            start.current + second - first.current
          }px)`;
        else
  ...
  useEffect(() => {
    ...
    body.addEventListener("dragstart", (e: React.DragEvent<HTMLDivElement>) =>
      e.preventDefault()
    );
    body.addEventListener("dragend", (e: React.DragEvent<HTMLDivElement>) =>
      e.preventDefault()
    );
    return () => {
      if (resize) {
  ...
  return (
    <div
      ref={lineRef}
      className={clsx(styles["resize-line-style"], {
...
export const ResizeBox: React.FC<ResizeBoxProps> = React.memo((props) => {
  ...
  const { width: calcBodyWidth, height: calcBodyHeight } = useResizeDetector({
    targetRef: bodyRef,
  ...
  useEffect(() => {
    if (!calcBodyWidth || !calcBodyHeight) return;
    specialSizeFun(isVer ? calcBodyHeight : calcBodyWidth);
    // 第一次进入时记录宽高 优化后续性能
    if (firstRenderRef.current) {
      perBodyWidth.current = calcBodyWidth;
      perBodyHeight.current = calcBodyHeight;
      firstRenderRef.current = false;
      return;
    }
    if (isVer) {
      if (perBodyHeight.current === calcBodyHeight) return;
      perBodyHeight.current = calcBodyHeight;
      setBodyHeight(calcBodyHeight);
    } else {
      if (perBodyWidth.current === calcBodyWidth) return;
      perBodyWidth.current = calcBodyWidth;
      setBodyWidth(calcBodyWidth);
    }
  }, [calcBodyWidth, calcBodyHeight]);
    ...
    >
      ...
      {freeze ? (
        <div
          ...
          className={clsx(styles["resize-split-line"], {
